    ...
    pub fn update(&mut self, delta_time: f32) {
        ...
        match self.current_occupation {
            Some(Occupation::Woodcutter) => {
                self.inventory.add_item(Item::new("Log".to_string(), "A Log".to_string()));
                self.jobs[0].experience += 1;
            }
            Some(Occupation::Miner) => {
                self.inventory.add_item(Item::new("Stone".to_string(), "A Stone".to_string()));
                self.jobs[1].experience += 1;
            }
            Some(Occupation::Farmer) => {
                self.inventory.add_item(Item::new("Wheat".to_string(), "Some Wheat".to_string()));
